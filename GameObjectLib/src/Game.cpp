...
void Game::Run() {
	...
	if (!pauseBackgroundTexture.loadFromFile("assets/images/pauseBackground.jpg")) {
		// Gérer l'erreur si le chargement échoue
	}

	sf::RectangleShape pauseBackground(sf::Vector2f(window->getSize().x, window->getSize().y));
	...
	while (window->isOpen())
	{
		...
		while (window->pollEvent(event))
		{
			...
			if (event.type == sf::Event::MouseButtonPressed) {
				if (gameState == PAUSE) {
					...
					pause.GetButtons();
					// Vérifier si le bouton 2 est cliqué
					if (pause.GetButtons()[1]->getComponent<Button>()->IsClicked(mousePos)) {
						// Ouvir options
					}

					// Vérifier si le bouton 3 est cliqué
					else if (pause.GetButtons()[2]->getComponent<Button>()->IsClicked(mousePos)) {
						window->close();
						//quitte le programme
					}
				}
		...
		if (gameState == PAUSE) {
			scene.setCamera(CreateCamera(1));
			scene.Update();
			HandleCamera(scene.getGamera(), player, map);
